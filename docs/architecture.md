...
The 2026-09-20 bidding calibration increased the weight of the bidder's own hand while keeping the base team-trick estimate close to half of 13. The continuation draw allows the first few bids to develop and reduces marginal overcalls after three bids or above level 9. This is a provisional behavioral calibration against 256 seeded auctions per profile, not a validated probability model: high contracts still fail often in full games. The CI strength fixture uses 32 seeds for speed; the optional report uses 128. See `docs/bot-evaluation.md` for the observed distribution and outcome limits.
